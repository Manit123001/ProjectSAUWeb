                ...
                if (picShow.Image != null)
                {
                    if (MessageBox.Show("Thanks For Join", "Complete !!", MessageBoxButtons.YesNo, MessageBoxIcon.Information) == DialogResult.Yes)
                    {
                        ...
                        //{
                            ...
                            picShow.Image.Save(@"Z:\" + name + ".jpg", ImageFormat.Jpeg);

                            string sql = "INSERT INTO tbexposau (name_image) VALUES ('" + tbName.Text + "')";
                           // MySqlConnection con = new MySqlConnection("Server=192.168.0.1; user=expoSAU; password=1234; database=images_expo");
                            MySqlConnection con = new MySqlConnection("Server=192.168.0.1; Database=images_expo; Uid=root; Pwd=password;");
                       ...
                       // catch(Exception ex)
                       // {
                       //     MessageBox.Show("ตรวจสอบชื่อที่ป้อนอีกครั้งว่าถูกต้องหรือไม่ หรือ ชื่อที่ป้อนต้องไม่มีเครื่องหมาย / * - + \\ ^ % " + ex.StackTrace, "Warning !!");
                       // }
